    ...
    processData(data, expand_tracker, configs: Configs, edit_tracker, internal_configs) {
        ...
        processed_data.map(rec => {
            ...
            rec.pathx = parent_pathx.join('.');
             //Checking if already expanded then maintaining it as is or else collapsing
             expand_tracker[rec.pathx]? expand_tracker[rec.pathx] = true:expand_tracker[rec.pathx] = false;
